...
# Host-side launcher for Jetson Nano fine-tuned EfficientAT + Wi-Fi bridge.
# Run on Jetson host, not inside Docker:
#   cd ~/efficientat_ws
#   ./run_wifi_bridge_finetuned_from_host.sh plughw:2,0 8765 2
...
# Used by jetson_wifi_bridge_finetuned.py through environment variables.
...
echo "DOA_NORTH_OFFSET=$DOA_NORTH_OFFSET"
...
sudo docker run --runtime nvidia -it --rm \
  ...
  bash -lc '
    ...
    echo "=== Disable broken Kitware apt repo if present ==="
