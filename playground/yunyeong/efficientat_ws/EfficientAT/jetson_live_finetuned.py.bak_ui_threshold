...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=0.30, help="softmax score threshold")
    ...
    while True:
        ...
        if display_db < min_db:
            if args.print_skipped:
                print("[%s] db=%.1fdB below %.1fdB, skipped" % (time.strftime("%H:%M:%S"), display_db, min_db), flush=True)
            continue
